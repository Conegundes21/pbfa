...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`h-full bg-dark-200 border border-dark-400 ${featured ? "border-nature-green/30" : ""} 
        rounded-xl overflow-hidden transition-all duration-300 hover:shadow-glow hover:shadow-${color}/20 relative`}
      >
        {featured && (
          <div className="absolute top-0 right-0">
            <div className="bg-nature-green text-dark text-xs font-semibold px-3 py-1 rounded-bl-lg">
              Destaque
            </div>
          </div>
        )}
        <div className="p-6">
          <div
            className={`w-14 h-14 flex items-center justify-center rounded-full bg-${color} mb-6 group-hover:scale-110 transition-transform duration-300`}
          >
            {icon}
          </div>
          <h3 className="text-xl font-bold mb-3 text-white group-hover:text-nature-green transition-colors duration-300">
            {title}
          </h3>
