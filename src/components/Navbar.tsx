        ...
        <div className="flex items-center justify-between py-4">
          ...
          <nav className="hidden md:flex items-center space-x-1">
            {navLinks.map((link, index) => (
              ...
              >
                {link.name}
                <span className="absolute bottom-0 left-0 w-full h-0.5 bg-nature-green scale-x-0 transition-transform duration-300 origin-left hover:scale-x-100"></span>
              </motion.a>
            ))}
          </nav>
            ...
            <div className="container mx-auto px-4 py-4">
              <nav className="flex flex-col space-y-4">
                {navLinks.map((link) => (
                  ...
                  >
                    <span>{link.name}</span>
                    <ChevronDown className="h-4 w-4 text-nature-green" />
                  </a>
                ))}
